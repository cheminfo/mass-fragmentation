...
const cidMed = [];
const cidHigh = [];
const dir = readdirSync(join(__dirname, 'hmdb/xml'));

// for loop to parse masses
for (let filename of dir) {
  const data = readFileSync(join(__dirname, 'hmdb/xml', filename));
  ...
  if (!parsed['ms-ms-peaks']) continue;
  let msmsPeaks = parsed['ms-ms-peaks']['ms-ms-peak'];
  if (!Array.isArray(msmsPeaks)) {
    msmsPeaks = [msmsPeaks];
  }

  const peaks = msmsPeaks.map((entry) => {
    return {
