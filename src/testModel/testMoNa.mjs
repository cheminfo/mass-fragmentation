...
const dataSet = JSON.parse(
  fs.readFileSync(`${__dirname}/../../data/monadb/testSet.json`),
);
const model = JSON.parse(fs.readFileSync('../training/model/modelMoNa.json'));
const solutions = JSON.parse(fs.readFileSync(`${__dirname}/....`));
...
for (let i = 0; i < dataSet.length; i++) {
  ...
  for (let f = 0; f < dataSet[i].smiles.length; f++) {
    let fragmentsResult = [];
    const smilesMoleculeTest = dataSet[i][0].smiles[f];
...
fs.writeFileSync(
  join(__dirname, 'resultsTestModelMoNa.json'),
